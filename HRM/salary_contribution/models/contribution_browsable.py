...
class ContributionBrowsableObject(BrowsableObject):
    """Đối tượng Browsable để truy cập dữ liệu đóng góp lương"""

    def __getattr__(self, attr):
        # Nếu attr tồn tại trong dict, trả về ContributionData
        if attr in self.dict:
            return ContributionData(self.employee_id, attr, self.dict[attr], self.env)
...
class ContributionData(BrowsableObject):
    """Đối tượng lưu trữ dữ liệu cho một loại đóng góp cụ thể"""

    def __init__(self, employee_id, code, data_dict, env):
        super().__init__(employee_id, data_dict, env)
        self.base_fields += ["code", "employee_contribution", "employer_contribution"]
        self.code = code

        # Xác định các thuộc tính từ data_dict
        if isinstance(data_dict, dict):
            self.employee_contribution = data_dict.get('employee_contribution', 0.0)
            self.employer_contribution = data_dict.get('employer_contribution', 0.0)
        else:
